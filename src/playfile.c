...
static int
flow (sox_effect_t * effp, sox_sample_t const *ibuf,
...
void playfile (misc_t *misc, audio_info_t *sound_devices,
               char *in_file, char *in_type, char *tempo)
{
  ...
  if (! (out = sox_open_write (sound_devices[misc->current_sink].device,
                  &in->signal, NULL, sound_devices[misc->current_sink].type,
                  NULL, NULL)))
  {
    int e;
    ...
    printf ("\n\n%s (%s): %s\n", sound_devices[misc->current_sink].device,
            sound_devices[misc->current_sink].type, strerror (e));
